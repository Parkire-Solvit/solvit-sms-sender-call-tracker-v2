...
function computeActivitySummary(events: any[], totalAgents: number = 0) {
  ...
  // Seed Realistic Simulation Scenario
  app.post("/api/seed-simulation", async (req, res) => {
    try {
      const demoAgents = [
        { name: "Kelvin Kimathi", tag: "Move Consultant", phone: "0712345001" },
        { name: "Sarah Mwangi", tag: "Customer Service", phone: "0712345002" },
        { name: "Brian Ochieng", tag: "Operations", phone: "0712345003" },
        { name: "Faith Wanjiku", tag: "Customer Service", phone: "0712345004" },
      ];
      ...
      for (const ag of demoAgents) {
        await db.execute(
            `INSERT INTO agents (name, phone_number, tag, last_active_at)
             VALUES (?, ?, ?, CURRENT_TIMESTAMP)
             ON CONFLICT(name) DO UPDATE SET tag = excluded.tag, last_active_at = CURRENT_TIMESTAMP`,
            [ag.name, ag.phone, ag.tag]
          );
        const record = await db.queryOne("SELECT id FROM agents WHERE name = ?", [ag.name]);
        if (record) agentMap.set(ag.name, record.id);
      }

      // Generate realistic timestamps for today (between 8:00 AM and current time)
      const now = new Date();
      const nairobiNow = new Date(now.getTime() + 3 * 60 * 60 * 1000);
      const todayStr = nairobiNow.toISOString().split("T")[0];

      // Helper to generate ISO UTC timestamp for given hour/minute today in Nairobi
      const makeTs = (hours: number, minutes: number) => {
        // Nairobi is UTC+3, so UTC hour = hours - 3
      ...
      const kelvinId = agentMap.get("Kelvin Kimathi");
      const sarahId = agentMap.get("Sarah Mwangi");
      const brianId = agentMap.get("Brian Ochieng");
      const faithId = agentMap.get("Faith Wanjiku");

      const seedEvents = [
        // 1. Inbound Missed Call -> Resolved by Kelvin in 8 mins (MET SLA)
        { agent_id: kelvinId, type: "CALL", target_phone: "0722100200", status: "MISSED", duration: 0, ts: makeTs(8, 30) },
        { agent_id: kelvinId, type: "CALL", target_phone: "0722100200", status: "OUTGOING", duration: 0, ts: makeTs(8, 38) },
        { agent_id: kelvinId, type: "CALL", target_phone: "0722100200", status: "CONNECTED", duration: 185, ts: makeTs(8, 39) },

        // 2. Inbound Missed Call -> Sarah answered callback in 14 mins (MET SLA)
        { agent_id: sarahId, type: "CALL", target_phone: "0733400500", status: "MISSED", duration: 0, ts: makeTs(9, 10) },
        { agent_id: sarahId, type: "CALL", target_phone: "0733400500", status: "OUTGOING", duration: 0, ts: makeTs(9, 24) },
        { agent_id: sarahId, type: "CALL", target_phone: "0733400500", status: "CONNECTED", duration: 92, ts: makeTs(9, 25) },

        // 3. Outgoing call by Brian -> Unanswered -> Follow-up SMS sent in 5 mins
        { agent_id: brianId, type: "CALL", target_phone: "0711999888", status: "NO_ANSWER", duration: 0, ts: makeTs(10, 0) },
        { agent_id: brianId, type: "SMS", target_phone: "0711999888", status: "SENT", duration: 0, ts: makeTs(10, 5) },

        // 4. Inbound Missed Call -> Faith breached (called back after 45 mins)
        { agent_id: faithId, type: "CALL", target_phone: "0799888777", status: "MISSED", duration: 0, ts: makeTs(10, 15) },
        { agent_id: faithId, type: "CALL", target_phone: "0799888777", status: "OUTGOING", duration: 0, ts: makeTs(11, 2) },
        { agent_id: faithId, type: "CALL", target_phone: "0799888777", status: "CONNECTED", duration: 140, ts: makeTs(11, 3) },

        // 5. Active OPEN Obligation (Incoming missed call 10 minutes ago, waiting for callback)
        { agent_id: kelvinId, type: "CALL", target_phone: "0700111222", status: "MISSED", duration: 0, ts: makeTs(Math.max(8, nairobiNow.getUTCHours() - 1), 45) },

        // 6. Connected Outbound Client Calls
        { agent_id: kelvinId, type: "CALL", target_phone: "0720555666", status: "OUTGOING", duration: 0, ts: makeTs(11, 15) },
        { agent_id: kelvinId, type: "CALL", target_phone: "0720555666", status: "CONNECTED", duration: 320, ts: makeTs(11, 16) },
        { agent_id: sarahId, type: "CALL", target_phone: "0723888999", status: "OUTGOING", duration: 0, ts: makeTs(12, 10) },
        { agent_id: sarahId, type: "CALL", target_phone: "0723888999", status: "CONNECTED", duration: 210, ts: makeTs(12, 11) },
      ];

      for (const ev of seedEvents) {
        await db.execute(
          `INSERT INTO events (agent_id, type, target_phone, status, duration, timestamp)
           VALUES (?, ?, ?, ?, ?, ?)`,
          [ev.agent_id, ev.type, ev.target_phone, ev.status, ev.duration, ev.ts]
        );
      }

      res.json({
        success: true,
        message: `Successfully seeded ${seedEvents.length} events for ${demoAgents.length} agents for date ${todayStr}.`,
        date: todayStr
      });
    } catch (err) {
      console.error("[API] Error in seed-simulation:", err);
