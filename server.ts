...
async function startServer() {
  ...
  if (emailRuntime) {
    const migration = await db.queryOne('SELECT version FROM schema_migrations WHERE version = 15');
    if (!migration) throw new Error('Email SLA requires database migration 015 before it can be enabled');
