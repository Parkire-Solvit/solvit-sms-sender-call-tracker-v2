...
export function EmailSlaReports({ employeeEmail, members, refreshToken }: { employeeEmail?: string; members: {email:string;display_name:string}[]; refreshToken: number }) {
  ...
  return <section className="space-y-5">
    ...
    {report && <>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">{[['Received',report.summary.received],['Response SLA',percent(report.summary.response.compliancePercent)],['Resolution SLA',percent(report.summary.resolution.compliancePercent)],['Older backlog open',report.summary.olderBacklogStillOpen]].map(([label,value]) => <div key={label} className="rounded-xl border border-slate-200 bg-white p-4"><p className="text-xs text-slate-500">{label}</p><p className="text-2xl font-semibold mt-2">{value}</p></div>)}</div>
      <div className="grid md:grid-cols-2 gap-4">{(['response','resolution'] as const).map(stage => { const s=report.summary[stage]; return <div key={stage} className="rounded-xl border bg-white p-5"><h4 className="font-semibold capitalize">{stage} outcomes</h4><dl className="grid grid-cols-2 gap-3 text-sm mt-3">{[['On time',s.met],['Completed late',s.completedLate],['Overdue & open',s.overdueOpen],['Within deadline',s.pendingWithinDeadline],...(stage === 'response' ? [['Closed without detected reply',s.closedWithoutCompletion]] : [])].map(([k,v]) => <div key={k}><dt className="text-slate-500 text-xs">{k}</dt><dd className="font-semibold">{v}</dd></div>)}</dl><p className="text-xs text-slate-500 mt-3">Median completion time: {s.medianWorkingMinutes === null ? '—' : emailDurationLabel(s.medianWorkingMinutes)}. Pending within deadline is not assessed; late completions, open breaches and closures without a detected reply count as misses.</p></div>; })}</div>
      <div className="rounded-xl border bg-white p-5 space-y-3"><h4 className="font-semibold">Owner breakdown</h4>
        ...
        {report.owners.map(o => <div key={o.email} className="grid sm:grid-cols-4 gap-2 rounded-lg bg-slate-50 p-3 text-sm"><div className="font-semibold break-words">{o.name}</div><div><span className="text-xs text-slate-500 block">Received</span>{o.received}</div><div><span className="text-xs text-slate-500 block">Response SLA</span>{percent(o.response.compliancePercent)} · {o.response.completedLate} late</div><div><span className="text-xs text-slate-500 block">Resolution SLA</span>{percent(o.resolution.compliancePercent)} · {o.resolution.completedLate} late</div></div>)}
      </div>
