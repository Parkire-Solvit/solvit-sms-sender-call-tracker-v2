...
export function EmailSlaReports({ employeeEmail, members, refreshToken }: { employeeEmail?: string; members: {email:string;display_name:string}[]; refreshToken: number }) {
  ...
  return <section className="space-y-5">
    ...
    {report && <>
      ...
      <div className="grid md:grid-cols-2 gap-4">{(['response','resolution'] as const).map(stage => { const s=report.summary[stage]; return <div key={stage} className="rounded-xl border bg-white p-5"><h4 className="font-semibold capitalize">{stage} outcomes</h4><dl className="grid grid-cols-2 gap-3 text-sm mt-3">{[['On time',s.met],['Completed late',s.completedLate],['Overdue & open',s.overdueOpen],['Within deadline',s.pendingWithinDeadline]].map(([k,v]) => <div key={k}><dt className="text-slate-500 text-xs">{k}</dt><dd className="font-semibold">{v}</dd></div>)}</dl><p className="text-xs text-slate-500 mt-3">Median completion time: {s.medianWorkingMinutes === null ? '—' : emailDurationLabel(s.medianWorkingMinutes)}. Pending within deadline is not assessed; late completions and open breaches count as misses.</p></div>; })}</div>
